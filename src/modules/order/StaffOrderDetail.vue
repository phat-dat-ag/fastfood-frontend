...
import { updateOrder, getOrder, } from '../../service/order.service';
...
async function handleConfirmOrder(orderId: number) {
    await useApiHandler<Order>(
        () => updateOrder(orderId, { status: ORDER_STATUS.CONFIRMED }),
        {
...
async function handleMarkDelivering(orderId: number) {
    await useApiHandler<Order>(
        () => updateOrder(orderId, { status: ORDER_STATUS.DELIVERING }),
        {
...
async function handleMarkDelivered(orderId: number) {
    await useApiHandler<Order>(
        () => updateOrder(orderId, { status: ORDER_STATUS.DELIVERED }),
        {
            loading: "Đang đánh dấu đơn hàng",
            error: "Lỗi đánh dấu đơn hàng",
...
async function handleCancelOrder(orderId: number) {
    ...
    if (!reason) return;
    await useApiHandler<Order>(
        () => updateOrder(orderId, { status: ORDER_STATUS.CONFIRMED, reason }),
        {
